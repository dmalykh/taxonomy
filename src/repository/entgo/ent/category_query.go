...
func (cq *CategoryQuery) sqlAll(ctx context.Context) ([]*Category, error) {
	...
	if query := cq.withChildren; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(category.ChildrenPrimaryKey[0], fks...))
			},
	...
	if query := cq.withParent; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(category.ParentPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "parent": %w`, err)
		}
		query.Where(category.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.Parent = append(nodes[i].Edges.Parent, n)
			}
		}
	}
